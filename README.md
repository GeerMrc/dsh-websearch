...
- **成员卡**（默认折叠，整卡头部点击展开）：状态绿点、名称、**Key 计数徽标**（仅展开态：0=灰空心圆，N=品牌色数字 pill，超 10=警示色；悬停显示 `已配置 N/10`，展开瞬间自动刷新计数）、未保存 pill、启停开关；展开区 = API Key 输入（明文输入/保存后 •••• 遮罩/清除）、Key 策略循环 chip、接口地址覆盖（留空=默认）、逐成员参数（对齐上游官方枚举，ⓘ 悬停有完整语义说明）。
