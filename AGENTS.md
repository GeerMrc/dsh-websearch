...
DSH 外挂式统一 WebSearch 管理插件（独立项目，零内核侵入）。技术栈一句话：TypeScript + cordis 插件协议 + vitest，宿主为 deepseek-harness（`dsh`）官方 npm 线——devDeps 钉版与生产供版树以 package.json 与 `/Volumes/IPFSJK/Zcode/dsh-ops-3080-runbook.md` 为正本（勿以本行为准；截至 S32 启动：devDeps=0.1.5-rc.2 线〔T3 刷新中〕、生产供版 016a1=dsh-v0.1.6-alpha.1）。
