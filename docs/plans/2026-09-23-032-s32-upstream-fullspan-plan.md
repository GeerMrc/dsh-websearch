...
> 阶段 2 独立审核（2026-09-23）：NEEDS REVISION → 已吸收全部 3 必改（T6 分线 scratch home 防 same-version skip；T3 增 AGENTS.md seam 行刷新+lockfile 同提交；minimumReleaseAgeExclude 改条件触发）+ 3 建议（全局 node22 前提、15→14 更正、.gitignore dist-artifacts/ 行）；复审定论见本文件 git 历史与 audit-log。审核实测另证：minimumReleaseAge 全局未配置（pnpm 11.7.0 `config get` = undefined），远端三 tag 均在。
...
- **C2 npm 官方现状（registry 实查 2026-09-23）**：`next`=0.1.5-rc.3（09-22）、0.1.6-alpha.2（09-17）、`alpha`=0.1.7-alpha.2（09-22）；cordis 4.0.3/4.0.4（09-22，latest=4.0.4）。插件 devDeps 14 条 dsh-\* 全钉 0.1.5-rc.2、cordis 4.0.2，对上述任何版本零演练。本地 clone tag 止于 dsh-v0.1.6-alpha.1（需 fetch；远端三 tag 经审核实测均在）。
...
- **端口方案**：本轮开发/调试/验证一律 **3434** + 分线 scratch home（`/tmp/dshws-s32/home-015rc3`、`/home-016a2`、`/home-017a2`——防同版本 tgz 换包 skip，三线各自全新 profile）；3423（websearch 常驻 dev，运行中）、3424（selfupdate U3 保留位）不动；3080 仅 T7 受控操作。
- **环境前提（全局）**：本机默认 node v20.18.3 下 pnpm 11.7.0 崩溃（`node:sqlite` 缺失，需 ≥22.13；engines 要求 ^22.19）——T2/T3/T5/T6 一切 pnpm/npm pack/宿主起服操作均在 nvm node 22.23.2（`~/.nvm/versions/node/v22.23.2/bin` 前置 PATH）下执行。
...
| T3 | 依赖域刷新（机械变更豁免候选，分类须独立 Agent 确认）：peer 六条 → `>=0.1.5-rc.1 <0.1.8 \|\| 0.1.6-alpha.1 \|\| 0.1.6-alpha.2 \|\| 0.1.7-alpha.1 \|\| 0.1.7-alpha.2`；devDeps 14 条 dsh-\* → 0.1.7-alpha.2；runtime dep `dsh-typert-protocol` → 0.1.7-alpha.2；cordis devDep 按 0.1.7-alpha.2 线实际 peer 升 4.0.3/4.0.4；AGENTS.md seam-依赖纪律段 peer/cordis 行同步刷新；`pnpm install`（minimumReleaseAge 经审核实测**未配置**，无需预防性扩展——仅当安装实际报 minimum release age 错误时补 exclude 并留痕）后全量门墙 | semver 判定脚本对 7 个已发布版本全 true（实测亲见）；typecheck（node+client）/lint/vitest/build/check:i18n 全绿 + 实测数字留痕；AGENTS.md seam 行与 package.json 一致；pnpm-lock.yaml 同提交 |
| T4 | 源码适配（条件触发，按 T2 矩阵；TDD 先红后绿，禁攒批）：候选——agent/created 类型增广、locale 键、client inject 面核对 | 每项 红→绿→commit；零适配则记录证据关闭 |
| T5 | 版本与产物：v0.1.1→v0.1.2；CHANGELOG（含 C1 历史纠偏披露）；`npm pack` 产物存持久路径 `/Volumes/IPFSJK/Zcode/dsh-websearch/dist-artifacts/`（.gitignore 显式加 `dist-artifacts/` 行——现有 `dist/` 模式不匹配）+ v0.1.1 回滚副本；docs/upgrade.md §1 版本表增 v0.1.2 行 + 三线演练矩阵与 3434 口径 | tgz 双副本在持久路径；CHANGELOG/upgrade.md 更新在档；.gitignore 生效 |
| T6 | 3434 三线演练（015rc3/016a2/017a2 各一轮，**分线 scratch home** home-015rc3/home-016a2/home-017a2 各自全新 profile init）：目标树 cwd 起服（DSH_HOME=分线 home，--port 3434 --no-open）→ plugin add v0.1.2 tgz → `--dump-config` 三钉扎行对照 → 凭据 gate（授权复制）→ 真实搜索一条 + served-by 徽标 → 降级验证 → web_fetch 两态 → GUI 冒烟（排序/折叠/兜底选择器）→ key-count 徽标（Typert RPC 跨版本关键点）→ §4 漂移防线四查（patch 钉扎/id 撞名/seam 形状/toolview 替身卡片）→ 停服换树（换树即换 home，无 same-version skip 面） | 三线证据链齐全（audit-logs）；终态 3434 无残留进程、凭据副本已删；任一不过即 blockers 如实上报 |
