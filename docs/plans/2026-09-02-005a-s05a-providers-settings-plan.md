...
- **热改通路（链核零改动）**：链核 `run()` 每次迭代读 `this.#options.order`、每成员读
  `perMemberTimeoutMs`（chain/core.ts）——getter-backed options 对象即可热改；成员 enabled
  gate 已是热读（S04 D3）。settings 源同样要过 `resolveConfig` 显式默认化（schema 零默认注入，
  S03 实锚）。
...
| T1 | 脚手架提取（D4）：`src/providers/shared.ts`（`isAbortError`/`throwIfMemberAborted`/`memberAborted`/`isPositiveInteger`/`unfoldHttpErrorDetail`/`resolveMemberApiKey`——错误族码与 label 参数化）+ deepseek/tavily 切换到共享 helper；**禁证红**：全量测试回归绿、断言与错误消息文本零漂移（label 保形） | 重构类验收 = 全量绿 + `git diff` 确认行为面零变化 + 覆盖未变 | 重构类 | T0 |
...
| T7 | apply 五成员定形（D2）：exa/perplexity/firecrawl 以 resolveConfig 成员配置实例化 + **显式传 gates**（Agent Note s04 §1 纪律）；firecrawl 双注册（search + fetch registry + ctx.web 双面）；链 options 改 getter-backed（order/perMemberTimeoutMs 热读）；`ctx.inject(['settings'])` 接 D1 live state（缺服务回退）；apply.test 升级：**五 provider 注册冒烟**（search=chain+5、fetch=chain-fetch+firecrawl）+ **热改链序实测下次搜索生效**（fake settings：改链序→onChange→spy 调用序翻转）+ timeout 热读断言 + enabled gate 翻转 + 无 settings 服务回退可跑 | 先红 → 后绿（全量回归）→ commit | TDD | T3/T4/T5/T6 |
| T8 | 门墙收口 + Agent Note + progress-M3 增补：环境验证输出（`node --version && pnpm --version` ≥22.19）+ 四命令全绿（完整输出过滤，禁 tail 截断）；Agent Note 落 docs/notes/（settings 热改通路/live state 形态/firecrawl v2 取证与双接口/提取边界）；progress-M3 增 S05a 批次表 + 已验锚点新增（settings seam 行号 / firecrawl docs URL / 上游双 provider 行号 / dsh-settings 发布线） | 环境输出 + 四命令绿 + Note 落盘 + 台账增补 | — | T7 |
...
- **R2** exa/perplexity 单测 mock HTTP 四态红→绿：成功/429/断网/超时（abort 传播）+ 请求映射 +
  ...
  available() + id 契约——每任务红/绿证据/commit 三元组
- **R3** firecrawl search + fetch 双面红→绿：search 四态 + data.web 映射；fetch 请求/映射
  （markdown→text、statusCode 透传）/402-429 错误/abort；单类双接口形态
...
| settings 热改 + 回退 | T6/T7 | 3 | 同上（链序 spy 序 + timeout/enabled + 缺服务回退） |
| 五 provider 注册冒烟 | T7 | 3 | apply.test 注册面断言 |
| 门墙四命令 | T8 收口 | 3 / 4 | 主 Agent 亲跑数字；阶段 4 独立 Agent 重放（全量唯一责任点） |
| R1-R5 对峙 | T9 | 4 | 独立 Agent 实测 |
| 冒烟重放 | T9 阶段 5 | 5 | 独立 Agent 亲跑一条/以用户路径驱动构建产物；采信阶段 3/4 数字 |
...
- **getter-backed options 与 ChainOptions 类型**：`order` getter 需满足 `readonly string[]` 面
  （对象字面量 getter 天然兼容 readonly 属性）；若 typecheck 有异议 → 改为显式
  `get order(): readonly string[]` 类形态；链核零改动为前提
