...
对 S05a **PASS**——🔴×0；🟡×1 新增（Y-1 = progress-M3 状态区未随 S05a 收官刷新，**家族
第三次复发**：S04 审核曾抓 progress-M1 同类、S05a 收官又漏自身——系统性确认，本棒 T0
清偿 + dont-do 第三条沉淀防复发）。

阶段 1 只读摸底实锚（2026-09-02 亲测，deepseek-harness dev@3281e04b59）：

- **dsh CLI 面**（apps/cli/src/bin.ts --help 实测）：`--dump-config` 打印组合后 profile 树并
...
- **组合层级**（profile.ts:125-140 + profile-boot.ts:130-140）：bundles patch（各 bundle 的
  `dsh.bundle.patch`）→ profile cordis.patch.yml（用户层）→ home 层 → `--patch` 覆盖——
  用户层 web 行标量后写生效 = 接线机制。
- **上游对照锚**（packages/bundle/base/cordis.patch.yml:449-454）：`web` 行
...
| T-prep | 本包构建交付物：`pnpm build` + `pnpm pack` → tarball 落 `/tmp/dshws-s05b/`（ADR-0007 交付形态）；pack 清单核对（lib + cordis.patch.yml + package.json） | tarball 在位 + 清单实测入记录 | 机械类 | T0 |
...
| T5 | 卸载复原（D3）：`dsh plugin --profile web remove dsh-websearch`（实测 bundles reconcile 行为）+ 删用户层两行 → `--dump-config` 与 **T1 基线 diff 为零**（searchProvider 回 deepseek-official）→ 复原对照成立；精确步骤实录 | dump diff 零 + 步骤原文 | 实测类 | T4 |
...
| T7 | 门墙收口 + Agent Note + progress-M3 增补：四命令全绿（本棒预期零产品代码变更——四命令 = 漂移哨兵；若 T0-T6 引发任何 src/tests 变更则按实测数字报）；Agent Note 落 docs/notes/（安装/接线/卸载精确命令实录 + reconcile 行为 + dump-config 观测面——S09 手册正素材）；progress-M3 增 S05b 批次表 + 已验锚点（PROFILE_TEMPLATES/dump-config/base patch 行号/S02 配方） | 四命令绿 + Note/台账增补 | — | T6 |
...
| T9 | 收尾 6 件套 + 原子翻转（STATUS 台账 ✅ + 当前位置块 + roadmap S05b 行 ✅ + **M3 行按 D4 处理** + progress-M3 验收表，同一序列）+ 踩坑沉淀 + 接力指令（S06；记录末节 + 回复末尾）+ `feat/s05b-install-e2e` **`--no-ff`** 合入 master | R5 全过 | — | T8 |
...
- **R5** scratch 隔离可证 + 门墙四命令 + 收尾 6 件套 + 原子翻转 + 分支闭环：全程 DSH_HOME
  指向 /tmp、端口非 3080、`~/.dsh` 零写入（命令清单）；with-key 槽位指引落盘（D5）
...
- **remove 的 reconcile 行为未知**（D3）：若 remove 只删 dependencies 不清 bundles → 复原
  验收仍以 dump diff 为零为准，手动清 bundles 行属精确卸载步骤（实录入 Note，S09 手册口径）
