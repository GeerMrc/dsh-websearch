...
- 负面/接受：将来获付费意愿或 Startups 计划需整棒恢复（**方向性声明而非可执行 runbook**〔阶段 5 注：移除横跨 4 commits + loopback 承载改写，revert 会撞测试——恢复需重放而非纯 revert〕）；统一语言入口只剩 Tavily 一家（语言维度覆盖收窄——API 客观边界）。观察：AnySearch 匿名免费额度达标但未挂可持续性钩子——上游收紧免费政策即触发准入复审。
