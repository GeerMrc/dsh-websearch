...
|---|---|---|---|---|---|
| **01** | 治理套件安装 + 架构/WBS 定稿过审 | 安装 11 模板并填占位符；AGENTS.md/架构/roadmap/ADR 过独立审核（F-001..F-019 修订）与用户批准 | ① `grep -r '{{' docs/ 仅命中活占位（`{{NN}}` 类）；② 独立审核复审结论 APPROVED 落盘；③ 用户批准记录在 session 记录；④ governance §3.1.1/§3.4 保号 | 1 天 | ⏳ |
