...
| **27** | 生产切换批（T7——用户 2.5 批准 2026-09-13 路线 B） | ①备份保留三件（.credentials.yaml/settings.yaml/.anonymous-user-id → /tmp/dshws-s27/backup + md5）②清除（sessions/storages/logs/attachments/profiles/bak——贴清单后执行）③worktree 装 dsh-websearch v0.1.0（显式 DSH_HOME）+ dump 三接线 + healing 指向 worktree 亲证④回填+凭据绿点⑤3080 起服+真实搜索 served-by⑥阶段 4/5+收官 | R1-R5 见 plan 027；主树零接触（merge-forward 降后续 housekeeping 棒） | 0.5 天 | 🚧 |
