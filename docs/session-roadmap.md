...
| **22** | P3 对齐批 + 全功能一致性深度审核（重排插行） | 开头独立 Agent 全参数深度审核（当日官方文档逐家 diff）；无计费风险参数：Tavily exact_match/start_date/end_date；Exa contents.text.verbosity/include·excludeSections；Firecrawl tbs 组合（sbd/cdr）/safe；AnySearch language/tag/params（棒内实测）；seam 阻挡 ~15 项维持 P4 边界披露 | 每参数红绿+守卫；全量门墙 exit0 | 1-2 天 | ⏳ |
