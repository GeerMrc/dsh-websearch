...
| **20** | P2 中价值参数批（S16 审计批次首次排入；**用户裁定 2026-09-10 先于 S15**——排期裁定原文在 session-20 记录） | 全局域名入口（searchIncludeDomains/ExcludeDomains 二选一 + validate-hook 双路径 fail-loud〔ADR-0018〕+ fan-out Tavily/Exa/Firecrawl 含守卫）+ Tavily 3（chunksPerSource/filterByLanguage/includeDomainsMode）+ Exa 2（category×日期守卫/contents.maxAgeHours）+ Firecrawl 3（sources news+web/categories/**search 面 timeout 修复**）+ AnySearch 零改动披露（上游面仅 query/max_results/zone） | 每参数红绿留痕 + 组合限制守卫逐条单测；全量门墙 exit0（缺省零 wire 漂移，排除 timeout/sources 两项有意漂移）；3423 冒烟 | 1-2 天 | 🚧 |
