...
| **19** | 完整移除 dshws-perplexity 成员（用户终裁插行：免费额度准入标准 ADR-0017） | 前后端+测试+文档全清（provider/config 节/GUI 卡/链序 5→4/errors 码族/locales）；fallbackMember legacy 别名归一 'auto'（存量兼容）；ADR-0017（成员准入标准：高可用+**有免费额度**+多 key+对齐上游）；ADR-0016 superseded；S18 真实实测闭合项 moot | grep -ri perplexity src/ tests/ 零输出（docs 历史档豁免）；legacy 归一红绿；全量门墙 exit0；3423 换包冒烟 | 0.5-1 天 | 🚧 |
