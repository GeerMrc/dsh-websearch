...
| **26** | 定档批：生产就绪规范化（**用户主计划批准 2026-09-12**——T6 确认后；T7 生产批下棒） | ①兜底行绿点前置（用户审核确认后交付）②lint 5 警告 + 三处治理口径勘正 + 🟢 顺手项（badge 死分支/re-export）③**版本定档 v0.1.0**（0.9.0 内部编号归历史档；此后小补丁走 0.1.x 补丁位、仅特大架构升 0.2.x）④S15 文档腿执行（plan 015 已批准：README zh/en + anysearch 迁移 + upgrade 手册 + architecture 刷新）⑤git 定档（59 已合分支全删 + tag v0.1.0 + 全历史敏感扫描；远端仅准备不推送） | R1-R5 见 plan 026；用户确认点 = T1 后暂停呈审 | 1 天 | 🚧 |
