...
| **18** | Perplexity Agent API 迁移（Sonar 日落应对；S17 阶段 1 调研发现插行，2.5 默认批准） | `/chat/completions` → `POST /v1/agent` responses 形态；参数同名迁移（search_recency_filter→web_search filters / search_context_size→web_search 工具 / max_tokens→max_output_tokens / user_location 同名；search_language_filter 无对应物——官方 drop 清单）；官方公告 Sonar 支持至 **2026-09-27**（docs.perplexity.ai 弃用横幅 + migrate-from-sonar 指南，出处见 plan 017 附录 A）；S17 已按可迁移形态设计 config 面 | 〔勘注 2026-09-10，2.5 降级验收裁定〕迁移后真实 API 实测按凭据可用面执行（无 PERPLEXITY key 时 wire 级 + e2e.real 自跳披露，补 key 后手动闭合）；config 面零改动验证；全量门墙 exit0；2026-09-27 前落地 | 1-2 天 | ✅ 2026-09-10（docs/sessions/2026-09-10-session-18.md；0.3.1；401\|13(414) exit0；config 零改动 git diff 亲证；死线解除）〔勘注 2026-09-10：本棒交付随 S19 成员移除转历史档——决策时点信息不同非浪费〕 |
| **19** | 完整移除 dshws-perplexity 成员（用户终裁插行：免费额度准入标准 ADR-0017） | 前后端+测试+文档全清（provider/config 节/GUI 卡/链序 5→4/errors 码族/locales）；fallbackMember legacy 别名归一 'auto'（存量兼容）；ADR-0017（成员准入标准：高可用+**有免费额度**+多 key+对齐上游）；ADR-0016 superseded；S18 真实实测闭合项 moot | grep -ri perplexity src/ tests/ 零输出（docs 历史档豁免）；legacy 归一红绿；全量门墙 exit0；3423 换包冒烟 | 0.5-1 天 | ✅ 2026-09-10（docs/sessions/2026-09-10-session-19.md；0.4.0；377\|11(388) exit0；准入标准 ADR-0017） |
| **20** | P2 中价值参数批（S16 审计批次首次排入；**用户裁定 2026-09-10 先于 S15**——排期裁定原文在 session-20 记录） | 全局域名入口（searchIncludeDomains/ExcludeDomains 二选一 + validate-hook 双路径 fail-loud〔ADR-0018〕+ fan-out Tavily/Exa/Firecrawl 含守卫）+ Tavily 3（chunksPerSource/filterByLanguage/includeDomainsMode）+ Exa 2（category×日期守卫/contents.maxAgeHours）+ Firecrawl 3（sources news+web/categories/**search 面 timeout 修复**）+ AnySearch 零改动披露（上游面仅 query/max_results/zone） | 每参数红绿留痕 + 组合限制守卫逐条单测；全量门墙 exit0（缺省零 wire 漂移，排除 timeout/sources 两项有意漂移）；3423 冒烟 | 1-2 天 | ✅ 2026-09-10（docs/sessions/2026-09-10-session-20.md；0.5.0；398\|11(409) exit0；守卫六条全档） |
| **21** | web_fetch 全模式多链接管（**用户批准 roadmap 重排 2026-09-10**——四决策：Exa 保留+fetch 链独立 / 两态开关 / 全败报错无 http 尾 / 计费面归未来迭代；主计划原文在 session-21 记录） | ①AnySearch /v1/extract 真实探针定案 ②Tavily /extract face（failed_results 语义/PDF URL/默认 markdown）③AnySearch extract face ④fetch 链接线（ChainFetchProvider 注册+firecrawl fetch 面+三家 gates；fetchChain 默认 [firecrawl,tavily,anysearch]——Exa 不进）⑤开关改造：FetchGateProvider 转运行时路由器（patch 不动；ON=链服务含 restrict 移除 / OFF=gate 内置 http 直抓〔官方行为等价近似——patch 钉死下官方实例永不被选，S1 口径〕）⑥GUI fetch 链配置面 ⑦测试三件适配 ⑧3423 实测 | 三家 fetch 真实降级链实测；ADR-0019；web_fetch 🟡 翻账；全量门墙 exit0；0.6.0 | 1-2 天 | ✅ 2026-09-10（docs/sessions/2026-09-10-session-21.md；0.6.0；415\|13(428) exit0；AnySearch extract 实测 1,048ms） |
| **22** | P3 对齐批 + 全功能一致性深度审核（重排插行）〔✅ 2026-09-10 S22 0.7.0；439\|13(452) exit0；正本 = docs/sessions/2026-09-10-session-22.md〕| 开头独立 Agent 全参数深度审核（当日官方文档逐家 diff）；无计费风险参数：Tavily exact_match/start_date/end_date；Exa contents.text.verbosity/include·excludeSections；Firecrawl tbs 组合（sbd/cdr）/safe；AnySearch language/tag/params（棒内实测）；seam 阻挡 ~15 项维持 P4 边界披露 | 每参数红绿+守卫；全量门墙 exit0 | 1-2 天 | ✅ |
| **22a** | Web Fetch 链 UI 修正批 0.7.1（用户四项反馈插行：chip wrap/区块迁移/更名/连点 conflict）〔✅ 2026-09-11；442\|13(455) exit0；正本 = docs/sessions/2026-09-11-session-22a.md〕| — | — | 0.5 天 | ✅ |
| **22b** | 设置页折叠化 0.7.2（用户反馈插行：接管卡成员卡式折叠 + 搜索链详细配置子折叠）〔✅ 2026-09-11；443\|13(456) exit0；正本 = docs/sessions/2026-09-11-session-22b.md〕| — | — | 0.5 天 | ✅ |
| **23** | UI/UX 全面对齐 DSH（重排插行；功能对齐完成后做）〔✅ 2026-09-11 S23 0.8.0；447\|13(460) exit0；双审两轮 PASS/COMPLETE；正本 = docs/sessions/2026-09-11-session-23.md〕| ①独立审核 Agent 出与宿主设置页排版/布局/交互一致性差距清单 ②实施对齐 ③独立再审核（用户要求双审） | 差距清单→逐项对齐→复审 PASS；浏览器实测 | 1 天 | ✅ |
| **23a** | 配置模型深研 + 真实 API 全面实测 + 成员卡 UI 重排（用户插行 2026-09-11；S15 顺延） | ①key 可达性探查（3423 环境）②逐成员逐参数真实 API 实测→对齐度矩阵 ③配置模型三路径研究报告呈用户裁定 ④成员卡 UI 重排方案+实施 | 实测矩阵每参数亲见数字；报告交付；UI 前后对照亲验 | 1 天 | ✅ 2026-09-11（0.8.1；STATUS 台账在案——本行为 S26 T3 勘正漏刷） |
| **23b** | Key 行内联 + 兜底入详细配置 0.8.2（用户反馈微批；配置模型裁定 C 落档）〔✅ 2026-09-11；448\|13(461) exit0；正本 = docs/sessions/2026-09-11-session-23b.md〕| — | — | 0.5 天 | ✅ |
| **25** | 采纳上游 0.1.5-rc.2：插件 0.9.0 适配 + 3423 分级上线（用户裁定 A；八项兼容清单全 PASS + 真实搜索验证）〔✅ 2026-09-12 **T6 人工验证 gate 呈用户**；正本 = docs/sessions/2026-09-12-session-25.md〕| — | — | 0.5 天 | ✅（至 T6） |
| **26** | 定档批：生产就绪规范化（**用户主计划批准 2026-09-12**——T6 确认后；T7 生产批下棒） | ①兜底行绿点前置（用户审核确认后交付）②lint 5 警告 + 三处治理口径勘正 + 🟢 顺手项（badge 死分支/re-export）③**版本定档 v0.1.0**（0.9.0 内部编号归历史档；此后小补丁走 0.1.x 补丁位、仅特大架构升 0.2.x）④S15 文档腿执行（plan 015 已批准：README zh/en + anysearch 迁移 + upgrade 手册 + architecture 刷新）⑤git 定档（59 已合分支全删 + tag v0.1.0 + 全历史敏感扫描；远端仅准备不推送） | R1-R5 见 plan 026；用户确认点 = T1 后暂停呈审 | 1 天 | ✅ 2026-09-13（v0.1.0 定档 + 阶段 4/5 PASS；正本 session-26 记录；**M5 文档腿 ✅ Tier B**） |
| **27** | 生产切换批（T7——用户 2.5 批准 2026-09-13 路线 B） | ①备份保留三件（.credentials.yaml/settings.yaml/.anonymous-user-id → /tmp/dshws-s27/backup + md5）②清除（sessions/storages/logs/attachments/profiles/bak——贴清单后执行）③worktree 装 dsh-websearch v0.1.0（显式 DSH_HOME）+ dump 三接线 + healing 指向 worktree 亲证④回填+凭据绿点⑤3080 起服+真实搜索 served-by⑥阶段 4/5+收官 | R1-R5 见 plan 027；主树零接触（merge-forward 降后续 housekeeping 棒） | 0.5 天 | ✅ 2026-09-13（横幅 0.1.5-rc.2 + served-by: dshws-tavily 生产实证；R1-R5 全 PASS；正本 session-27 记录） |
| **24** | DSH 上游 0.1.5-rc.2 升级预演（用户指令；生产零接触）〔✅ 2026-09-12 预演完成**待用户决策 A/B/C**；正本 = feat/s24-upgrade-015 分支 docs/notes/2026-09-12-s24-upgrade-rehearsal.md + /tmp/dshws-s24/upgrade-assessment-report.html〕| — | — | 0.5 天 | ✅（预演） |
| **15** | README + 迁移 + 升级手册〔勘注 2026-09-11：plan 015 已批准（cd3f341），因用户插行 S23a 顺延——S23a 收官后按原计划执行〕（原 S12 行；2026-09-04 重排编号）〔勘注 2026-09-10：用户裁定 fetch+全对齐+UI/UX 优先——顺延至 S21-S23 之后收尾〕 | README（zh/en：安装/配置/GUI/链语义/**多 key 单槽逗号值**/**装即接管与卸载复原 ADR-0013**）；anysearch 迁移（**ADR-0013 口径：卸 anysearch〔其 bundle 钉扎退出〕→装本插件〔钉扎即生效〕；用户层两行仅用于否决/自定义/fetch 可选，整段替换须重述两键**；共存/退役语义 ADR-0009+0013）；docs/upgrade.md 升级演练手册（含溯源替身卡片维护点 ADR-0010 + 漂移防线检查项 ADR-0013）；〔S23 勘注：素材面新增 plan 023 §0 豁免裁定清单（D5/D6/D7/D11 + D12 方言锚定 + U3 词典全面对照）+ 0.8.0 对齐批 CHANGELOG〕 | 由独立审核 Agent 照手册从零在 scratch profile 走通安装→搜索并留痕；演练手册步骤可独立执行 | 1 天 | ✅ 2026-09-13〔补账勘正 2026-09-23 S32 T1：实质由 S26 T6 文档腿（README zh/en + upgrade + architecture v2.0，Tier B PASS）+ S27 生产重建实测（worktree 装 v0.1.0 → dump 三接线 → 真实搜索 served-by 留痕）复合兑现；行留存作历史索引〕 |

**里程碑 M5 交付就绪**：e2e 收口全绿，文档自洽可复现 —— ✅ 2026-09-13（e2e 收口腿 S08 + 文档腿 S26 Tier B + 真实搜索实证 S27 生产重建；2026-09-23 S32 T1 补账勘正）

| **32** | 上游全跨度升级适配批（用户指令 2026-09-23：冲突全面审核+随官方最新升级；三裁定=全跨度/修死 tgz 树不切/授权凭据复制） | 正本 plan 032 + ADR-0021 + Note s32 三线矩阵；T0-T9 十任务链 | R1-R11 见 plan 032 §2（阶段 4 全 PASS + 阶段 5 COMPLETE-WITH-NOTES 八必修全落） | 1 天 | ✅ 2026-09-23（v0.1.2 定版；B 线工具腿 BLOCKED-by-upstream 留档；证据 docs/sessions/2026-09-23-session-32.md） |

| **33** | 随上游 0.1.7-rc.1/rc.2 迭代适配批（用户指令 2026-09-26：多版本全跨度+fork 修复审计+多端发布+生产切换呈批） | 正本 plan 033（rev1 APPROVED）+ Note s33 矩阵；T0-T9 + T2b/T7 前置门 | R1-R11 见 plan 033 §2（含网络受阻如实记录条款） | 1 天 | 🚧 2026-09-26（进行中） |
