...
| 凭据 | credentials 服务按 ref 解析（四层：进程环境 > `~/.dsh/.credentials.yaml` > 项目 .env > 用户 .env），每操作解析=热生效；`credentials/updated` 事件 | `inject: ['credentials']` |
| 设置 | `settings.installSection(ctx, ns, Config, entry, opts)` 命名空间节，GUI 经 `settings.mutate` 写入 | `inject: ['settings']` |
...
│   ├── credentials.ts          # 每 provider key ref 解析 + available() 缓存 + credentials/updated 刷新
│   └── settings.ts             # installSection：链序等用户可热改字段
├── client/                     # client 半区（设置页）
│   ├── index.ts                # apply：ctx.slots.inject('settings.section', ...)
│   ├── WebSearchSection.tsx    # 「网页搜索」设置页：provider 卡 + 优先级排序 + 状态
...
- `available()`（便宜本地检查，禁网络调用）：≥1 个启用成员的凭据就绪（credentials describe 缓存 + `credentials/updated` 事件刷新）→ true。
- `search(request)`：按序遍历成员——未注册/未启用/凭据未配置 → 跳过；`available()`=false → 跳过；**运行期抛错（429 限额/网络/宕机等）→ 记录后降级下一成员**；某成员成功 → 返回其结果，`servedBy` 记录实际服务方 id（附于结果 meta，工具结果透出）。
- 全部成员耗尽 → 抛最后一名成员的错误（末端 fail-loud），错误信息含逐成员失败摘要。
- 用户经组合标量钉死单个具体 provider（如 `searchProvider: dshws-tavily`）时走该 provider 直连，**不降级**（显式指定 = 用户意志）。
- fetch 链（`dshws-chain-fetch`）同构。
...
## 8. 开放问题（S02 spike 定谳，ADR-0006 承接）
...
| ADR-0002 | 完整高可用链语义（选择时跳过 + 运行失败降级 + 全败末端报错） |
| ADR-0003 | 全 provider 收编单插件 + `dshws-` id 前缀隔离 |
| ADR-0004 | v1 全局优先级（settings 落点，home 级共享）；per-profile 留 YAML patch |
| ADR-0005 | 完全中立开箱：上游钉死由用户层 patch 覆盖，插件默认链为空序时的内置默认序 |
