...
export const en: Record<DshWsLocaleKey, string> = {
  ...
  keySelRandom: 'Random',
  keySelectionHint: 'Keys default to "{policy}" selection; on failure the tool retries across its keys first (up to 3) before degrading to the next tool.',
...
export const zh: Record<DshWsLocaleKey, string> = {
  ...
  keySelRoundRobin: '轮询',
  keySelRandom: '随机',
  keySelectionHint: 'key 默认按「{policy}」选取；请求失败优先在本工具的多把 key 间重试（至多 3 次），仍失败才降级下一个工具。',
