...
export const en: Record<DshWsLocaleKey, string> = {
  ...
  searchIncludeDomainsNote: 'Comma-separated allowlist (e.g. example.com,foo.org) applied to Tavily/Exa/Firecrawl. Wildcards work on Exa only (Firecrawl skips them). Mutually exclusive with the exclude list — setting one clears the other.',
...
export const zh: Record<DshWsLocaleKey, string> = {
  ...
  searchIncludeDomainsNote: '逗号分隔白名单（如 example.com,foo.org），作用于 Tavily/Exa/Firecrawl。通配符仅 Exa 支持（Firecrawl 跳过）。与排除列表互斥——设置其一自动清除另一。',
