...
export const en: Record<DshWsLocaleKey, string> = {
  ...
  keySelRandom: 'Random',
  keySelectionHint: 'Keys are drawn by "{policy}"; on a failed request the member redraws another key (up to 3) before degrading to the next chain member.',
...
export const zh: Record<DshWsLocaleKey, string> = {
  ...
  keySelRoundRobin: '轮询',
  keySelRandom: '随机',
  keySelectionHint: 'key 按「{policy}」选取；请求失败先在本工具内换 key 重试（至多 3 把），仍失败才降级下一成员。',
